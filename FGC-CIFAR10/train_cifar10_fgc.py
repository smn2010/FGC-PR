...
from src.datasets import cifar10_instance_dataloader
from models.get_models import get_res_for_cifar10

model_names = ['resnet20', 'resnet32', 'resnet56']

parser = argparse.ArgumentParser(description='PyTorch CIFAR10 FGC Training')
...
################## optimzer parameters ##################
parser.add_argument('--start-epoch', default=100, type=int, metavar='N',
...
################## resume parameters ##################
parser.add_argument('--bas_pretrain', default=None, type=str, metavar='BPATH',
                    help='path to batch-shaping initialization checkpoint (default: none)')
parser.add_argument('--resume', default=None, type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')

################## feature-gate coupling parameters. ##################
...
parser.add_argument('--nn_epoch', type=int, default=20, help='warm up epochs.')
parser.add_argument('--nn_num', type=int, default=5, help='number of nearest neighbours.')
parser.add_argument('--kappa', type=float, default=0, help='contrastive loss scalar parameter')
parser.add_argument('--contrast_index', type=int, nargs='+', required=True, help='index of fgc blocks') 
...
##################### Data loading code #######################
    ...
    train_loader, val_loader = cifar10_instance_dataloader(args)
...
######################## create model #############################
    ...
    print("=> initializing feature and gate queues.")
    model.initialize_contrast_settings(args.contrast_index, len(train_loader.dataset), args.nn_num, args.nn_epoch)
...
######################## pretrain or resume ########################
# optionally load the batch-shaping pretrained checkpoint
    if not args.resume and args.bas_pretrain:
        if os.path.isfile(args.bas_pretrain):
            print("=> loading batch-shaping pretrained checkpoint '{}'".format(args.bas_pretrain))
            if args.gpu is None:
                checkpoint = torch.load(args.bas_pretrain)
            else:
                # Map model to be loaded to specified single gpu.
                loc = 'cuda:{}'.format(args.gpu)
                checkpoint = torch.load(args.bas_pretrain, map_location=loc)
                
            args.start_epoch = checkpoint['epoch']
            model.load_state_dict(checkpoint['state_dict'], strict=False)

            print("=> loaded batch-shaping pretrained checkpoint '{}' (epoch {})"
                  .format(args.bas_pretrain, checkpoint['epoch']))
        else:
            print("=> no batch-shaping pretrained checkpoint found at '{}'".format(args.bas_pretrain))

##############################################################
# optionally resume from a checkpoint
    if args.resume and not args.bas_pretrain:
...
def train(train_loader, model, criterion, optimizer, epoch, args, f_log):
    ...
    on_epoch = epoch - args.start_epoch + 1
    
    if epoch <= args.l0_start:
        rho = 0.
    elif epoch <= args.l0_end:
        rho = (epoch - args.l0_start) * args.rho / (args.l0_end - args.l0_start)
    else:
        rho = args.rho
    
    kappa = args.kappa
    
    print('Training Epoch: {epoch} Lr: {lr:.2e} Kappa: {kappa:.4f} '
          'Rho: {rho:.4f}'.format(epoch=epoch, lr=optimizer.param_groups[0]['lr'], 
                                  kappa=kappa, rho=rho))
    if f_log is not None:
        print('Training Epoch: {epoch} Lr: {lr:.2e} Kappa: {kappa:.4f} '
            'Rho: {rho:.4f}'.format(epoch=epoch, lr=optimizer.param_groups[0]['lr'], 
                                    kappa=kappa, rho=rho), file=f_log)
    ...
    train_loss_ct  = AverageMeter('Loss_ct', ':.4f')
    train_loss_l0  = AverageMeter('Loss_l0', ':.4f')
    train_sparsity = AverageMeter('Sparsity', ':.4f')
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, top1, train_loss_cls, train_loss_ct, train_loss_l0, train_sparsity],
    ...
    for i, (images, target, indexes) in enumerate(train_loader):
        ...
        if rho == 0:
            loss_l0 = torch.tensor([0.]).cuda(args.gpu)
        else:
            loss_l0 = rho * model.get_loss_l0()
                
        ## contrastive loss
        ## if kappa == 0 or on_epoch <= args.nn_epoch:
        if kappa == 0:
            loss_ct = torch.tensor([0.]).cuda(args.gpu)
        else:
            loss_ct = kappa * model.get_loss_ct(indexes, on_epoch)
        
        loss = loss_cls + loss_ct + loss_l0
        ...
        train_loss_ct.update(loss_ct.item(), images.size(0))
